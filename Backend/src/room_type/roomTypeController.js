  ...
  try {
    ...
    if (roomTypeId == "") {
      res.status(http.StatusNoContent).json({
        message: "Failed to insert room type. Room type's id cannot be empty",
        error: error.message,
      });
      return;
    }
    if (type == "") {
      res.status(http.StatusNoContent).json({
        message: "Failed to insert room type. Room type cannot be empty",
        error: error.message,
      });
      return;
    }
    if (description == "") {
      res.status(http.StatusNoContent).json({
        message: "Failed to insert room type. Description cannot be empty",
        error: error.message,
      });
      return;
    }
    if (price == "") {
      res.status(http.StatusNoContent).json({
        message: "Failed to insert room type. Price cannot be empty",
        error: error.message,
      });
      return;
    }
    ...
    res
      .status(500)
      .json({ message: "Failed to insert room types", error: error.message });
  }
};
...
exports.updateRoomType = async (req, res) => {
  try {
    ...
    if (snapshot.empty) {
      res
        .status(200)
        .json({ message: "Room type not found", error: null, data: null });
      return;
    }
    ...
    res.status(200).json({
      ...
      error: null,
      data: newDataRoomType,
    });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Failed to update room type", error: error.message });
  }
};
...
exports.deleteRoomType = async (req, res) => {
  try {
    ...
    res
      .status(401)
      .json({ message: "Failed to delete room type", error: error.message });
  }
};
