...
exports.insertETicket = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: "E-ticket inserted successfully",
      error: null,
      data: eTicketData,
    });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Failed to insert e-ticket", error: error.message });
  }
};
...
exports.getETicket = async (req, res) => {
  try {
    ...
    if (snapshot.empty) {
      res
        .status(200)
        .json({ message: "No e-ticket found", error: null, data: null });
      return;
    }
    ...
    res
      .status(401)
      .json({ message: "Failed to get e-ticket", error: error.message });
  }
};
